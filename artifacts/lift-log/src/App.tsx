...
import { DashboardPage, HistoryPage, ProfilePage, ProgrammeDetailPage, ProgrammePage, WorkoutPage } from '@/pages';
...
function StartWorkoutRoute() {
  ...
  if (isError) return <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6"><h1 className="font-display text-3xl uppercase">Couldn’t start session</h1><button className="mt-4 rounded-md bg-primary px-4 py-3 text-sm font-bold text-primary-foreground" onClick={() => setLocation('/')}>Back to today</button></div>;
  return <div className="space-y-3"><div className="h-28 animate-pulse rounded-xl bg-muted" /><p className="font-data text-xs uppercase tracking-widest text-muted-foreground">Setting up your session…</p></div>;
}
